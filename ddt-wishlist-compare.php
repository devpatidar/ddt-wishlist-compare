<?php
/**
 * Plugin Name:       DDT Woocommerce Wishlist Compare
...
function dwc_activation_functions() {
	...
	$table_name = $wpdb->prefix . 'dwc_wishlist';
...
function d2_action_woocommerce_single_product_summary() { 
    echo '<div class="d2_wc_button">';
    	if (dwc_get_wishlist_options('dwc_single_wishlist_btn')) {
    		do_shortcode('[dwc_add_to_wishlist_button button_text='.dwc_get_wishlist_options('dwc_shop_wishlist_btn_text').']');
    	}
    	if (dwc_get_compare_options('dwc_single_compare_btn')) {
...
function d2_action_woocommerce_after_shop_loop_item() {
    ...
    echo '<div class="d2_wc_button">';
    	if (dwc_get_wishlist_options('dwc_shop_wishlist_btn')) {
    		do_shortcode('[dwc_add_to_wishlist_button button_text='.dwc_get_wishlist_options('dwc_shop_wishlist_btn_text').']');
    	}
    	if (dwc_get_compare_options('dwc_shop_compare_btn')) {
    		do_shortcode('[dwc_add_to_compare_button]');
    	}
...
add_action( 'wp_enqueue_scripts', 'd2_add_theme_scripts' );